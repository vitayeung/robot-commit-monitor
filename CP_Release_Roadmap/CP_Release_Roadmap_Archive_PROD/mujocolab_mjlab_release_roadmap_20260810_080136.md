...
- mjlab 在过去一年从早期 beta 版本快速演进为稳定的 GPU 加速机器人学习框架，核心定位是提供轻量级、可组合的环境和最小化设置摩擦。框架已从 v0.1.0 发展到 v1.5.3，依赖链从需要手动指定 git 修订版变为完全通过 PyPI 安装，大幅降低了用户入门门槛。
...
- v1.5.3 是 v1.5.2 之后的快速修复版本，主要解决大规模域随机化在特定 GPU 架构上的 CUDA 700 崩溃问题，并修复了射线传感器调试可视化滞后问题。同时新增了 Viser 奖励条面板的术语上限可配置性，以及多个训练流程的稳定性修复。这表明框架在 v1.5.0 的大版本升级后持续进行稳定性巩固和边缘情况修复。
...
- **域随机化从手动字段操作到类型化、自动一致的系统**：v1.2.0 用类型化的 `dr` 模块替换了旧的 `randomize_field` 接口，自动处理物理一致性（如质量变化后自动更新惯性）。v1.4.0 和 v1.5.0 持续扩展随机化覆盖范围，新增材质属性、几何对摩擦、以及每轴随机化事件的正确组合。v1.5.2 修复了多环境域随机化中 `actuator_acc0` 的 CUDA 非法内存访问问题。v1.5.3 进一步修复了大规模域随机化在特定 GPU 架构上的 CUDA 700 崩溃。这对 sim-to-real 转移的质量至关重要。

- **地形系统从基础生成到预设化、确定性课程**：v1.3.0 引入 `@terrain_preset` 装饰器和预设配置，v1.5.0 确保课程难度在行间确定且达到配置端点，并修复了多个边缘情况。v1.5.3 修复了 `terrain_levels_vel` 课程在初始重置时忽略 `max_init_terrain_level=0` 的问题。这使得复杂地形训练可复现，对四足和人形机器人运动策略训练是核心能力。

- **执行器建模从基础 PD 到完整的电机模型**：v1.3.0 简化了执行器配置（延迟内联化、XML 类型自动检测），v1.4.0 添加原生隐式 PD 执行器，v1.5.0 引入完整的直流电机执行器（支持电压/位置/速度模式、反电动势、热模型、LuGre 摩擦）。v1.5.1 新增 `MeshCfg` 网格编辑能力和 `SimulationCfg.broadphase` 碰撞配置，并修复了融合执行器命令延迟的共享问题。这使 mjlab 能够更准确地模拟真实机器人硬件。

## 版本演进解读

### v1.5.3（2026-07-22）

- 将 `mujoco-warp` 升级至 3.10.0.3，修复了大规模域随机化在特定 GPU 架构上引发的 CUDA 700 崩溃。同时修复了射线传感器调试可视化滞后一步的问题。
- Viser 奖励条面板的术语上限现在可通过 `ViewerConfig.reward_bar_max_terms` 配置，默认值为 20，环境超过 20 个奖励项时不再静默丢弃。修复了 `terrain_levels_vel` 课程在初始重置时忽略 `max_init_terrain_level=0` 的问题，以及速度任务中 `joint_pos` 观测未被正确偏置的问题。
- 破坏性变更：无。建议所有使用 v1.5.x 的用户升级到此版本以获得稳定性改进和新的可配置性。

### v1.5.2（2026-07-18）

- 修复了多环境域随机化中 `set_const` 触发时的 CUDA 非法内存访问问题，`actuator_acc0` 现在被正确扩展。修复了 `MaterialCfg.reflectance` 在构建 MuJoCo 模型时被忽略的问题。
- 这些修复直接影响了大规模并行训练（4096+ 环境）的稳定性，确保域随机化在批量模拟中不会导致崩溃。
- 无破坏性变更，建议所有使用 v1.5.x 的用户升级到此版本以获得稳定性改进。

### v1.5.1（2026-07-16）

- 将最低 `mujoco-warp` 要求提升至 3.10.0.2，修复了向量化环境中 `qfrc_constraint` 填充错误的问题（issue #1086）。新增 `MeshCfg` 网格编辑能力，允许按名称匹配网格资产并编辑其属性；新增 `SimulationCfg.broadphase` 和 `broadphase_filter` 用于配置宽阶段碰撞算法和过滤器。
- 融合执行器（理想 PD、直流电机）的命令延迟现在跨所有共享延迟配置的融合执行器应用统一的每环境滞后，修复了 issue #1035。修复了 `TorchArray` 在 `mujoco_warp` 3.10.0.2 下不将世界共享模型字段扩展到 `nworld` 的问题，以及 `mdp.bad_orientation` 在 float32 舍入导致 `acos` 输入超出 [-1, 1] 时返回 NaN 的问题。
- 破坏性变更：不再支持 `mujoco-warp` 3.10.0.2 之前的版本。建议用户更新依赖并重新测试训练流程。
...
- **采用时机**：mjlab 已从 v1.0.0 开始稳定，依赖链已完全通过 PyPI 管理。对于需要 GPU 加速机器人学习框架的团队，当前版本（v1.5.3）已具备生产级能力，包括完整的域随机化、确定性地形课程和多种执行器模型。建议新项目直接从 v1.5.3 开始，以获取最新的稳定性修复和可配置性改进。

- **迁移成本**：从 v1.2.0 到 v1.5.0 的每个主要版本都包含破坏性变更。域随机化接口在 v1.2.0 完全重写，执行器配置在 v1.3.0 简化，XML 执行器配置类合并。如果从 v1.1.x 或更早版本迁移，需要重写域随机化代码和执行器配置。v1.4.0 和 v1.5.0 的破坏性变更较小（移除 `multiccd` 标志、分割数据类型变更）。v1.5.1 要求 `mujoco-warp` >= 3.10.0.2，v1.5.2 和 v1.5.3 无破坏性变更。建议参考每个版本的 changelog 进行逐步迁移。
...
- **生态系统观察点**：v1.5.0 的 research.rst 列出了多个基于 mjlab 的衍生项目（Asimov 双足机器人、H1 运动、MyoSuite 肌肉骨骼集成、LEAP Hand 灵巧操作等），表明社区正在围绕框架构建工具链。v1.5.1 新增的 `MeshCfg` 和碰撞配置接口进一步扩展了框架的定制能力。v1.5.3 新增的 `ViewerConfig.reward_bar_max_terms` 可配置性表明框架正在响应社区反馈进行精细化改进。建议关注 `unitreerobotics/unitree_rl_mjlab` 和 `pal-robotics/pal_mjlab` 等官方仓库的更新，它们反映了框架在真实机器人平台上的应用方向。
